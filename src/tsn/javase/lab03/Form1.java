...
// нахождение максимального элемента во второй строке.
        max = Integer.parseInt(jTable1.getValueAt(0, 0).toString());
        maxI = 0;
        maxJ = 0;
        for (int i = 0; i < jTable1.getRowCount(); i++) {
            for (int j = 0; j < jTable1.getColumnCount(); j++) {
                if (Integer.parseInt(jTable1.getValueAt(1, j).toString()) > max) {
                    max = Integer.parseInt(jTable1.getValueAt(1, j).toString());
                    maxI = 1;
                    maxJ = j;
                }
            }
       ...
       // сумма
        int sum2 = 0;
         for (int jj = 0; jj < jTable1.getRowCount(); jj++) {
                sum2 += Integer.parseInt(jTable1.getValueAt(jj, 1).toString());
        ...
        for (int i = 0; i < jTable1.getRowCount(); i++) {
            for (int j = 0; j < jTable1.getColumnCount(); j++) {
                if (Integer.parseInt(jTable1.getValueAt(i, j).toString()) == max) {
                    jTable1.setValueAt(sum2, maxI, maxJ);
                }
            }
        }

        jLabel_MAX.setText("Максимальный элемент 2 cтроки: " + max + 
                " [" + (maxI + 1) + "," + (maxJ + 1) + "] Cумма второго столбца: " + sum2);
